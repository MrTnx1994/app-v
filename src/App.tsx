...
export default function App() {
  ...
  const [dashboardPeriodDays, setDashboardPeriodDays] = useState<number>(30);
  ...
  useEffect(() => {
    if (user && !authLoading) {
      loadSalesStatistics(undefined, true, dashboardPeriodDays);
    }
  }, [dashboardPeriodDays]);
  useEffect(() => {
  ...
  const loadSalesStatistics = async (dateToExclude?: string, silent: boolean = false, days: number = dashboardPeriodDays) => {
    try {
      if (!silent) setLoading(true);
      const targetDate = dateToExclude || formattedDate;
      const [resSales, resIncoming, resInvoiceStats] = await Promise.allSettled([
        apiFetch(`/api/statistics/sales?endDate=${encodeURIComponent(targetDate)}&days=${days}`),
        apiFetch(`/api/statistics/incoming?endDate=${encodeURIComponent(targetDate)}&days=${days}`),
        apiFetch(`/api/statistics/invoices?endDate=${encodeURIComponent(targetDate)}&days=${days}`)
      ]);
  ...
  const getProductSalesInPeriod = (p: Product, days: number) => {
    ...
    const avgDaily = dashboardPeriodDays > 0 ? (historicalSales / dashboardPeriodDays) : 0;
  ...
  const getProduct30DayIncoming = (p: Product) => {
    ...
    const avgDaily = dashboardPeriodDays > 0 ? (historicalIncoming / dashboardPeriodDays) : 0;
    return avgDaily * dashboardPeriodDays + todayIncoming;
  };
