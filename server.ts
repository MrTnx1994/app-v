...
async function startServer() {
  ...
  db.prepare("UPDATE users SET password = ? WHERE email = ?").run(bcrypt.hashSync("010203", BCRYPT_ROUNDS), "GHANI@system.com");
  db.prepare("UPDATE users SET password = ? WHERE email = ?").run(bcrypt.hashSync("010203", BCRYPT_ROUNDS), "KHANI@system.com");
  console.log("Force reset password for GHANI@system.com, KHANI@system.com to 010203");
  ...
  if (!userExists) {
    ...
    // Force reset password to 010203 for the user
    db.prepare("UPDATE users SET password = ? WHERE email = ?").run(bcrypt.hashSync("010203", BCRYPT_ROUNDS), userEmailToSeed);
    console.log(`Force reset password for ${userEmailToSeed} to 010203`);
  }
  ...
  app.get("/api/statistics/sales", requireAuth, requireRole("admin", "sales", "driver", "visitor", "production_manager"), (req, res) => {
    try {
      const { endDate, days } = req.query as { endDate?: string; days?: string };
      ...
      const rows = (endDate ? stmtDailyPlansBeforeDesc.all(endDate, limitDays) : stmtDailyPlansAllDesc.all(limitDays)) as { date: string; data: string }[];
      rows.forEach((row) => {
        try {
  ...
  app.get("/api/statistics/invoices", requireAuth, requireRole("admin", "sales", "driver", "visitor", "production_manager"), (req, res) => {
    try {
      ...
      const rows = (endDate ? stmtDailyPlansUpToDesc.all(endDate, limitDays) : stmtDailyPlansAllDesc.all(limitDays)) as { date: string; data: string }[];
      rows.forEach((row) => {
        try {
          if (!row || !row.data) return;
          const dayData = JSON.parse(row.data);
          ...
          backup.dailyPlans.forEach((entry: any) => {
            ...
            if (!existingRow) {
              ...
              try {
                ...
                const existingInvoiceIds = new Set(existingData.invoices.map((i: any) => i && i.id));
                let addedAny = false;
