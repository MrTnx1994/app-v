...
async function startServer() {
  ...
  const isFreshDatabase = !fs.existsSync(DB_FILE_PATH);
  ...
  const stmtGetAllUsers = db.prepare("SELECT uid, email, role, status, driverName FROM users");
  const stmtInsertUser = db.prepare("INSERT INTO users (uid, email, password, role, status, driverName) VALUES (?, ?, ?, ?, ?, ?)");
  const stmtUpdateUserRole = db.prepare("UPDATE users SET role = ?, driverName = ? WHERE uid = ?");
  const stmtUpdateUserPassword = db.prepare("UPDATE users SET password = ? WHERE uid = ?");
  ...
  app.post("/api/users/create", requireAuth, requireRole("admin"), async (req, res) => {
    try {
      ...
      stmtInsertUser.run(uid, email, hashed, role, "active", driverName || "");
      res.json({ status: "success", user: { uid, email, role, status: "active", driverName: driverName || "" } });
    } catch (err: any) {
  ...
  app.post("/api/users/update", requireAuth, requireRole("admin"), async (req, res) => {
    try {
      ...
      if (password) {
        stmtUpdateUserPassword.run(await bcrypt.hash(password, BCRYPT_ROUNDS), uid);
      }
